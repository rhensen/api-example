...
SELECT station_id, strftime("%Y", date) as year, AVG(max_temp) as avg_max_temp,AVG(min_temp)as avg_min_temp,SUM(precip) as total_precip from weather group BY station_id, strftime("%Y", date);
"""
resultSet=conn.execute(create_summary_table).all()
def make_weather_row(row):
            weather=summaryWeather(station_id=row[0],year=row[1],
                avg_max_temp= row[2],
                avg_min_temp= row[3],
                total_precip= row[4])
            return(weather)
